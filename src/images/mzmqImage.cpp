...
constexpr size_t headerSize =
    256;  ///< total size in bytes of the header,  gives room to grow!

constexpr size_t nameSize = 128;         ///< The size of the name field.
constexpr size_t typeOffset = nameSize;  ///< Start of data type field
constexpr size_t size0Offset =
    typeOffset + sizeof(uint8_t);  ///< start of size0 field
constexpr size_t size1Offset =
    size0Offset + sizeof(uint32_t);  ///< start of size1 field
...
constexpr size_t xrifDifferenceOffset =
    tv_nsecOffset + sizeof(uint64_t);  ///< The XRIF encoding differencing
                                       ///< method.  Generally must be PIXEL.
constexpr size_t xrifReorderOffset =
    xrifDifferenceOffset +
    sizeof(int16_t);  ///< The XRIF encoding reordering method.
constexpr size_t xrifCompressOffset =
    xrifReorderOffset +
    sizeof(int16_t);  ///< The XRIF encoding compression method.
constexpr size_t xrifSizeOffset =
    xrifCompressOffset + sizeof(int16_t);  ///< The size of the compressed data.

constexpr size_t endOfHeader =
    xrifSizeOffset + sizeof(uint16_t);  ///< The current end of the header.
constexpr size_t imageOffset = headerSize;

static_assert(endOfHeader <= imageOffset,
              "Header fields sum to larger than reserved headerSize");

mzmqImage::mzmqImage() { m_ZMQ_context = new zmq::context_t(1); }
...
std::string mzmqImage::imageName() { return m_imageName; }

void mzmqImage::imageServer(const std::string &server) { m_server = server; }

void mzmqImage::imagePort(int port) { m_port = port; }

void mzmqImage::timeout(int to) { m_timeout = to; }

uint32_t mzmqImage::nx() { return m_nx; }

uint32_t mzmqImage::ny() { return m_ny; }

double mzmqImage::imageTime() { return m_imageTime; }
...
int mzmqImage::imageThreadStart() {
  try {
    ...
    std::cerr << std::string("exception in image thread startup: ") + e.what()
              << " " << __FILE__ << " " << __LINE__ << "\n";
    return -1;
  } catch (...) {
    std::cerr << "unknown exception in image thread startup " << __FILE__ << " "
              << __LINE__ << "\n";
    return -1;
  }
...
void mzmqImage::imageThreadExec() {
  ...
  while (!m_timeToDie) {
    ...
    while (!m_timeToDie && !reconnect)  // Inner loop waits for each new image
                                        // and processes it as it comes in.
    {
      zmq::message_t msg;
      ...
      try {
        ...
        if (m_timeToDie) break;  // This will true be if signaled during
                                 // shutdown
      ...
      if (msg.size() <= headerSize)  // If we don't get enough data, we
      ...
      if (nx != new_nx || ny != new_ny || atype != new_atype ||
        ...
        xrif_set_difference_method(
            xrif, *((int16_t *)(raw_image + xrifDifferenceOffset)));
        xrif_set_reorder_method(xrif,
                                *((int16_t *)(raw_image + xrifReorderOffset)));
        xrif_set_compress_method(
            xrif, *((int16_t *)(raw_image + xrifCompressOffset)));
      ...
      if (Nrecvd >= 10) {
        t1 = get_curr_time() - t0;
        std::cerr << m_imageName << " averaging " << Nrecvd / t1
                  << " FPS received.\n";
      }
#endif
...
float mzmqImage::pixel(size_t n) { return pixget(m_data, n); }
...
float mzmqImage::fpsEst() { return m_fpsEst; }
